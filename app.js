(() => {
  ...
  const state = {
    ...
    eraQuery: '',
    countryVisible: [],
    globe: { rotation: -0.65, tilt: 0.18, zoom: 1, dragging: false, lastX: 0, lastY: 0, projected: [], hover: null },
  ...
  function findItem(id) { return database.find(i => i.id === id) || atlas.find(i => i.atlas_id === id || i.id === id); }
  ...
  function renderCountryPanel() {
    ...
    if ($('#countryScope')) {
      $('#countryScope').textContent = country
        ? `Showing ${state.countryVisible.length} of ${allForCountry.length} poem${allForCountry.length === 1 ? '' : 's'}${state.poet !== 'all' ? ` by ${state.poet}` : ''}${state.countryQuery ? ` matching “${state.countryQuery}”` : ''}.`
    ...
    $('#countryResults').innerHTML = state.countryVisible.length
      ? `<div class="country-initial-group"><h4>${escapeHTML(code)}</h4>${state.countryVisible.map((p, index) => `<button class="country-result" data-open="${escapeHTML(itemId(p))}">
  ...
  function renderEraResults() {
    ...
    const list = (q ? allForEra.filter(p => itemHaystack(p).includes(q)) : allForEra).slice(0, 220);
    ...
    if ($('#eraScope')) $('#eraScope').textContent = `Showing ${list.length} of ${allForEra.length} poem${allForEra.length === 1 ? '' : 's'}${state.eraQuery ? ` matching “${state.eraQuery}”` : ''}.`;
    if ($('#eraResults')) {
      $('#eraResults').innerHTML = list.map(p => `<button class="era-poem-card" data-open="${escapeHTML(itemId(p))}"><span>${escapeHTML(dateOf(p))}</span><strong>${escapeHTML(p.title)}</strong><p>${escapeHTML(previewLines(p, 2))}</p><em>${escapeHTML(eraLabelForItem(p))}</em></button>`).join('') || '<p class="empty-mini">No poems match this era search.</p>';
    }
  }
  ...
  function initGlobe() {
    ...
    canvas.addEventListener('pointerdown', e => { state.globe.dragging = true; state.globe.lastX = e.clientX; state.globe.lastY = e.clientY; canvas.setPointerCapture(e.pointerId); });
    canvas.addEventListener('pointermove', e => {
      if (state.globe.dragging) {
        state.globe.rotation += (e.clientX - state.globe.lastX) * 0.006;
        state.globe.tilt += (e.clientY - state.globe.lastY) * 0.003;
    ...
    canvas.addEventListener('pointerup', e => { state.globe.dragging = false; const n = nearest(e); if (n) selectCountry(countries.find(c => c.code === n.code)); });
  ...
  function bindEvents() {
    ...
    $('#countryPoemSearch')?.addEventListener('input', e => { state.countryQuery = e.target.value.trim(); renderCountryPanel(); });
    $('#clearCountrySearch')?.addEventListener('click', () => { state.countryQuery = ''; if ($('#countryPoemSearch')) $('#countryPoemSearch').value = ''; renderCountryPanel(); });
    $('#eraPoemSearch')?.addEventListener('input', e => { state.eraQuery = e.target.value.trim(); renderEraResults(); });
    $('#clearEraSearch')?.addEventListener('click', () => { state.eraQuery = ''; if ($('#eraPoemSearch')) $('#eraPoemSearch').value = ''; renderEraResults(); });
    $('#resetAtlas').addEventListener('click', () => { state.country = null; state.era = 'all'; state.poet = 'all'; state.countryQuery = ''; state.globe.zoom = 1; state.globe.rotation = -0.65; state.globe.tilt = 0.18; if ($('#countryPoemSearch')) $('#countryPoemSearch').value = ''; renderEraRail(); renderEraResults(); renderPoetNav(); renderCountrySelect(); renderCountryPanel(); });
    document.addEventListener('click', e => {
      const era = e.target.closest('[data-era]');
      if (era) { state.era = era.dataset.era; renderEraRail(); renderEraResults(); renderCountryPanel(); }
      const poet = e.target.closest('[data-poet]');
      if (poet) { state.poet = poet.dataset.poet; renderPoetNav(); renderCountryPanel(); }
